...
export default function DinoLane({ dino, obstacles, images, frame }: DinoLaneProps) {
	...
	useEffect(() => {
		...
		const draw = () => {
			...
			if (frame === -1) {
				...
				if (img.complete) ctx.drawImage(img, 40, canvas.height - 48 - dino.y, 48, 48)
			} else if (dino.lean) {
				const img = (frame % 2 === 0) ? images.dinoLean1 : images.dinoLean2
				if (img.complete) ctx.drawImage(img, 40, canvas.height - 48 - dino.y, 64, 48)
			} else {
				const img = dino.y > 0 ? images.dinoStanding : ((frame % 2 === 0) ? images.dinoRun1 : images.dinoRun2)
				if (img.complete) ctx.drawImage(img, 40, canvas.height - 48 - dino.y, 48, 48)
			}
	...
	return <canvas ref={canvasRef} width={800} height={100} className="w-full bg-game-dark pixel-border" />
}
