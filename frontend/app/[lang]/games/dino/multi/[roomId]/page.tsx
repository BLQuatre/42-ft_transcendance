...
export default function DinoGamePage() {
    ...
    useEffect(() => {
        if (!accessToken) {
            console.error(dict?.games.dino.multi.errors.noToken || "Access token is undefined")
            return
        }

        if (!userRef.current) {
            console.error(dict?.games.dino.multi.errors.noUser || "User data not yet defined")
            return
        }

        if (!roomId) {
            console.error(dict?.games.dino.multi.errors.noRoom || "Room ID is undefined")
            return
        }

        // Prevent creating multiple connections
        if (socketRef.current && socketRef.current.readyState !== WebSocket.CLOSED) {
            console.log(dict?.games.dino.multi.errors.connectionExists || "WebSocket connection already exists")
            return
        }

        console.log(dict?.games.dino.multi.connection.creating || "Creating new WebSocket connection")

        const socket = new WebSocket("wss://localhost/api/ws/dino")
        socketRef.current = socket

        socket.addEventListener("open", () => {
            console.log(dict?.games.dino.multi.connection.connected || "Connected to game server")
        ...
        socket.addEventListener("message", (event) => {
            try {
                ...
                if (msg.type === "state") {
                    ...
                    if (!msg.room) {
                        console.error(dict?.games.dino.multi.errors.noRoomData || "No room data received")
                        return
                    }
                    ...
                    const transformedRoom: GameRoomType = {
                        ...
                        players: msg.room.players.map((p: any) => ({
                            id: p.id.toString(),
                            name: p.name || `${dict?.games.dino.defaultPlayerName || "Player"} ${p.id}`,
                            avatar: null,
                            isReady: p.isReady,
                            isYou: p.id === playerId
                        })) as Player[]
                    ...
                    console.log(dict?.games.dino.multi.connection.gameStarting || "Game starting!")
        ...
        socket.addEventListener("close", (event) => {
            console.log(dict?.games.dino.multi.errors.connectionClosed || "WebSocket connection closed:", event.code, event.reason)
        ...
        // Cleanup on unmount
        return () => {
            console.log(dict?.games.dino.multi.connection.cleaning || "Cleaning up WebSocket connection")
