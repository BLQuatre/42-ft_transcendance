...
export default function PongGamePage() {
	...
	useEffect(() => {
		const intervalId = setInterval(() => {
			...
			if (predictedY > canvasRef.current!.height)
				predictedY =
					canvasRef.current!.height - (predictedY - canvasRef.current!.height);
			if (predictedY < 0) predictedY = -predictedY;

			// Use prediction, unless ball is already going straight to the paddle
			bot.knownBallY =
				ball.x > (canvasRef.current?.width ?? 800) - 100 &&
				Math.abs(ball.vy) < 1
					? ball.y
					: predictedY;
		}, 1000);
	...
	useEffect(() => {
		...
		const handleInput = () => {
			...
			// player
			if (keysPressed.up) game.get_Lplayer().move(true);
			if (keysPressed.down) game.get_Lplayer().move(false);
			...
			if (bot.inputs.up) game.get_Rplayer().move(true);
			if (bot.inputs.down) game.get_Rplayer().move(false);
		};
									...
									<div className="absolute inset-0 flex flex-col items-center justify-center z-20">
										<div className="text-white text-center">
											<h2 className="text-3xl font-bold mb-4 animate-pulse">
												{dict?.games?.pong?.pauseInstructions ||
													"Press P to play/pause"}
											</h2>
											<div className="text-sm opacity-80">
												<p>
													{dict?.games?.pong?.moveInstructions ||
