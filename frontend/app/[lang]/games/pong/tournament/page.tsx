...
export default function PongGamePage() {
	...
	useEffect(() => { // update upcomingMatches every time bracketData is updated
	...
	const launchGame = () => {
		...
		const liveMatch = tournamentData.upcomingMatches.find(match => match.isLive);
		if (!liveMatch) return;
	...
	const updateTournamentAfterGame = (finalScores: { left: number, right: number }) => {
		if (!tournamentData) return;
		...
		// Determine winner
		const winner = finalScores.left === CONST.SCORE_WIN ? liveMatch.player1.name : liveMatch.player2.name;
		...
		setTournamentData(prev => {
			...
			const advanceWinner = (rounds: any[]) => {
				...
				for (let i = 0; i < rounds.length; i++) {
					const matchIndex = rounds[i].matches.findIndex((m: any) => m.id === liveMatch.id);
					if (matchIndex !== -1) {
						currentRoundIndex = i;
				...
				if (currentRoundIndex !== -1 && currentRoundIndex < rounds.length - 1) {
					...
					if (nextMatchIndex < nextRound.matches.length) {
						...
						nextRound.matches[nextMatchIndex] = {
							...nextMatch,
							player1: isFirstPlayerSlot ? { name: winner, score: null } : nextMatch.player1,
							player2: !isFirstPlayerSlot ? { name: winner, score: null } : nextMatch.player2
						};
					}
		...
		return (
			<div className="min-h-screen bg-background">
				...
				<div className="relative">
					<TournamentView tournamentData={tournamentData} validateNames={validateNames} launchGame={launchGame} />
					
					{/* Tournament Winner Overlay */}
					{tournamentWinner && (
						<>
							{/* Blur overlay - only applies to content below navbar */}
							<div className="absolute inset-0 z-40" style={{ backdropFilter: "blur(8px)" }}></div>
							
							{/* Fixed overlay that covers entire viewport */}
							<div className="fixed inset-0 z-50 flex flex-col items-center justify-center overflow-hidden" style={{ top: '64px' }}>
