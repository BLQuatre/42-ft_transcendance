...
export default function UserProfilePage() {
	...
	const getFriendStatus = (
		...
		const blocked = blockedUsers.find(
			(blocked) =>
				blocked.receiver_id === targetUserId ||
				blocked.sender_id === targetUserId
		);
		...
		const fetchUserProfile = async () => {
			try {
				...
				if (historyResponse.data && historyResponse.data.history) {
					...
					const activityData = processGameActivityData(
	...
	const processScoreData = (allHistory: any[]) => {
		...
		return scoreProgression.length
			? scoreProgression
			: [{ name: "Week 1", score: 0 }];
	};
		...
		return (
			<div className="min-h-screen bg-background flex flex-col">
				...
				<div className="flex-1 container py-8 px-4 md:px-6 flex flex-col items-center justify-center">
					<h1 className="font-pixel text-2xl mb-4 uppercase">
						{dict.profile.notFound.replace("%user%", userId)}
					</h1>
					<Button
						asChild
					...
					<div className="flex flex-col md:flex-row gap-6 items-center">
						<div className="relative">
							<Avatar className="h-24 w-24 md:h-32 md:w-32 border-4 border-game-blue">
								...
								/>
								<AvatarFallback className="font-pixel text-xl">
						...
						<div className="flex-1 text-center md:text-left flex flex-col justify-center align-middle">
							<h1 className="font-pixel text-2xl md:text-3xl">
							...
							{!isOwnProfile && (
								<>
									{friendStatus === FriendRequestStatus.ACCEPTED ? (
										<Button
											className="font-pixel bg-game-green hover:bg-game-green/90 uppercase"
										...
										<Button
											disabled
											className="font-pixel bg-yellow-500 hover:bg-yellow-500/90 uppercase"
										>
										...
										<Button
											className="font-pixel bg-green-500 hover:bg-green-600 uppercase"
						...
						<div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
							<Card>
								...
								<CardContent>
									...
									<p className="font-pixel text-xs text-muted-foreground uppercase mt-2.5">
										{dict.common.total}
									</p>
								</CardContent>
							</Card>
					...
					<TabsContent value="history" className="space-y-4">
						<div className="grid gap-6 md:grid-cols-2">
							<Card>
								<CardHeader>
									<CardTitle className="font-pixel text-sm">
										{dict.profile.sections.history.pong.title}
									</CardTitle>
									<CardDescription className="font-pixel text-xs">
										{dict.profile.sections.history.pong.description}
									</CardDescription>
								</CardHeader>
								<CardContent>
										...
										{gameHistory.pong.length > 0 ? (
											gameHistory.pong.map((gameResult, index) => {
												...
												return (
													<div
														...
														onClick={() =>
															handleMatchClick("pong", gameSession)
														}
													>
											...
											<div className="flex items-center justify-center h-64">
												<p className="font-pixel text-sm text-muted-foreground">
													{dict.profile.sections.history.noGames}
												</p>
											</div>
										)}
									</div>
							...
							<Card>
								<CardHeader>
									<CardTitle className="font-pixel text-sm">
										{dict.profile.sections.history.dino.title}
									</CardTitle>
									<CardDescription className="font-pixel text-xs">
										{dict.profile.sections.history.dino.description}
									</CardDescription>
								</CardHeader>
								<CardContent>
									<div className="space-y-3 h-[480px] overflow-y-auto pr-2 show-scrollbar">
											...
											gameHistory.dino.map((gameResult, index) => {
												...
												return (
													<div
														...
														className="flex justify-between items-center p-2 bg-muted rounded-md cursor-pointer hover:bg-muted/80 transition-colors"
														onClick={() =>
															handleMatchClick("dino", gameSession)
														}
													>
														<div className="flex items-center space-x-2">
															<div
