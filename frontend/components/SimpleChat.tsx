...
export function SimpleChat() {
	...
	const [showBlockedMessages, setShowBlockedMessages] =
	...
	const loadBlockedUsers = async () => {
		try {
			...
			if (response.data.friends) {
				const blockedUserIds = response.data.friends.map(
	...
	useEffect(() => {
		if (
			isOpen &&
			(activeView === ChatView.GENERAL || activeView === ChatView.PRIVATE)
		) {
			setTimeout(() => {
	...
	useEffect(() => {
		...
		if (
			generalSocket.current &&
			generalSocket.current.readyState !== WebSocket.CLOSED
		)
			return;
	...
	useEffect(() => {
		...
		window.addEventListener(
			"openPrivateChat",
			handleOpenPrivateChat as EventListener
		);
				...
				>
									...
									>
										<Avatar className="h-6 w-6">
											<AvatarImage
												...
												alt={activeFriend.name || "Player"}
											/>
											<AvatarFallback className="font-pixel text-[10px]">
												{(activeFriend.name || "Player").substring(0, 2)}
											</AvatarFallback>
										</Avatar>
										<span className="font-pixel text-sm hover:underline">
											{activeFriend.name || "Player"}
										</span>
										<span
								...
								<>
									...
									<span className="font-pixel text-sm">
										{activeView === ChatView.GENERAL
											? dict?.chat?.generalChat
											: dict?.chat?.privateMessages}
									</span>
								</>
							)}
						</div>
						...
						<div className="flex items-center space-x-1">
							...
							{activeView === ChatView.GENERAL && (
								<Button
									...
									title={
										showBlockedMessages
											? dict?.chat?.hideBlockedMessages
											: dict?.chat?.showBlockedMessages
									}
								>
									{showBlockedMessages ? (
					...
					<div className="border-b px-1 py-1 flex justify-center bg-muted/30">
						<div className="flex w-full gap-1">
							<Button
								variant={
									activeView === ChatView.GENERAL ? "secondary" : "ghost"
								}
								size="sm"
								...
								onClick={() => {
									...
									if (
										generalSocket.current &&
										generalSocket.current.readyState === WebSocket.OPEN
									) {
										generalSocket.current.send(
							...
							<Button
								variant={
									activeView === ChatView.FRIENDS ? "secondary" : "ghost"
								}
								size="sm"
								className="h-8 px-2 flex-1"
								onClick={() => {
							...
							<CardContent className="flex-1 p-0 overflow-hidden">
								<ScrollArea className="h-full p-3">
									<div className="space-y-3">
										{activeView === ChatView.GENERAL
											? filterBlockedMessages(generalMessages).map(
													(message) => (
														<div
															key={message.id}
															className={cn(
														...
														>
															...
															>
																...
																<div className="font-pixel text-xs mt-0.5 break-words whitespace-pre-wrap">
											...
											: activeFriend &&
												friendsMessages[activeFriend.id]?.map((message) => (
													<div
														key={message.id}
														className={cn(
															"flex mb-2",
				...
				<DialogContent className="sm:max-w-[425px]">
					<DialogHeader>
						<DialogTitle className="font-pixel text-sm">
							{dict?.chat?.inviteFriendToPlay?.replace(
								"%friend%",
								activeFriend?.name || dict?.chat?.aFriend
							)}
						</DialogTitle>
