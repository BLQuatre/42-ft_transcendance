...
export function ConsultDataDialog({
	...
	const totalWins = [...pongGameHistory, ...dinoGameHistory].filter(
		(game) => game.is_winner
	).length;
										...
										<CardContent className="space-y-6">
											<div className="flex items-center gap-6">
												...
												<div className="space-y-2">
													<h3 className="font-pixel text-2xl text-game-blue">
													...
													<div className="flex gap-2">
														{user?.tfaEnable && (
															<Badge
																variant="default"
																className="font-pixel text-xs bg-game-green"
															>
											...
											<div className="grid grid-cols-2 gap-6">
												<div className="space-y-2">
													<div className="flex items-center gap-2">
														...
														<p className="font-pixel text-sm text-muted-foreground uppercase">
															{dict.dialogs.consultData.profile.accountCreated}
														</p>
													</div>
													<p className="font-pixel text-lg">
														{user?.created_at
															? formatDate(user.created_at)
															: dict.dialogs.consultData.profile.unknown}
													</p>
													{user && (
														<p className="font-pixel text-xs text-muted-foreground">
															{user.created_at
																? `${Math.floor((Date.now() - new Date(user.created_at).getTime()) / (1000 * 60 * 60 * 24))} days ago`
																: dict.dialogs.consultData.profile
																		.noCreationDate}
														</p>
													)}
												</div>
												<div className="space-y-2">
													<div className="flex items-center gap-2">
														...
														<p className="font-pixel text-sm text-muted-foreground uppercase">
															{dict.dialogs.consultData.profile.accountUpdated}
														</p>
													</div>
													<p className="font-pixel text-lg">
														{user?.updated_at
									...
									<div className="h-full overflow-y-auto pr-2">
										<div className="space-y-4">
											<div className="p-4 bg-muted/50 rounded-lg border">
												<h4 className="font-pixel text-sm uppercase mb-2 text-game-blue">
													{dict.dialogs.consultData.rawData.userInfo}
												</h4>
