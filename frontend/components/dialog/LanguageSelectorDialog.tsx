...
const languages: { code: string; name: string; nativeName: string; flag: string }[] = [
	{
		code: "en",
		name: "English",
		nativeName: "English",
			...
			<DialogContent className="sm:max-w-lg bg-game-dark rounded-lg shadow-2xl">
				<DialogHeader className="text-center space-y-3">
					...
					<DialogDescription className="font-pixel text-center text-sm text-muted-foreground">
						{dict?.language?.description || "Select your preferred language for the interface"}
					</DialogDescription>
				</DialogHeader>
							...
							>
								<div className="flex items-center space-x-3 w-full">
									...
									<div className="flex-1 text-left">
										<div className="font-pixel text-sm uppercase">{language.nativeName}</div>
										<div className="font-pixel text-xs text-muted-foreground">{language.name}</div>
									</div>

									{isSelected && <Check className="h-4 w-4 text-game-blue flex-shrink-0" />}
				...
				<div className="text-center">
					<p className="font-pixel text-xs text-muted-foreground">
						{dict?.language?.savedPreference || "Language preference will be saved for future visits"}
					</p>
				</div>
			</DialogContent>
		</Dialog>
