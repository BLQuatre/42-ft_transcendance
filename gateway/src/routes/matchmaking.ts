...
export const matchmakingRoutes: FastifyPluginAsync = async (fastify) => {
    fastify.get('/ws/matchmaking', { websocket: true }, async (connection, req) => {
		const upstream = new WebSocket(`http://${process.env.MATCHMAKING_HOST}:${process.env.MATCHMAKING_PORT}`);
		...
		connection.on('message', (msg: any) => {
			try {
				...
				console.log(`isUpstreamOpen = ${isUpstreamOpen ? 'true' : 'false'} / (upstream.readyState === 1) = ${(upstream.readyState === 1) ? 'true' : 'false'}`)
		...
		upstream.on('close', (code, reason) => {
			const message = `Matchmaking closed. Code: ${code}, Reason: ${reason}`;
