...
export const tfaVerify = async (req: FastifyRequest, reply: FastifyReply) => {
	...
	const res = await axios
		.get(
			`http://${process.env.USER_HOST}:${process.env.USER_PORT}/user/secret/${id}`
		)
		.catch((err) => {
			if (err.response)
				...
				return reply
					.code(500)
					.send({ message: "Internal server ", statusCode: 500 });
		});
	...
	const response = await axios
		.get(`http://${process.env.USER_HOST}:${process.env.USER_PORT}/user/${id}`)
		.catch((err) => {
			if (err.response)
				return reply.code(err.response.status).send({ ...err.response.data });
			else
