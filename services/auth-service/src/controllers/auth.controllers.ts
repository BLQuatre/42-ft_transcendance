...
export const accessAuthentication = async ( req: AuthRequest, reply: FastifyReply) => {
    ...
    try {
        ...
        if (err instanceof TokenExpiredError)
            return reply.code(401).send({ message: 'Access token expired', statusCode: 401});
...
export const refreshAuthentication = async (req: AuthRequest, reply: FastifyReply) => {
    ...
    try {
        ...
        return reply.code(200).send({ message: 'Refresh token authentified', statusCode: 200, accessToken, refreshtoken});
    }
    catch (err){
        if (err instanceof TokenExpiredError)
            return reply.code(401).send({ message: 'Refresh token expired', statusCode: 401});
        else if (err instanceof JsonWebTokenError)
            return reply.code(401).send({ message: 'Invalid refresh token', statusCode: 401});
        else
            return reply.code(500).send({ message: 'Refresh token verification failed', statusCode: 500});
    }
}