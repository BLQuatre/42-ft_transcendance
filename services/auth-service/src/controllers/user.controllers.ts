...
export const register = async (
	...
	const response = await axios
		.post(`http://${process.env.USER_HOST}:${process.env.USER_PORT}/user`, {
			name: savedUser.name,
	...
	if (response) {
		const accessToken = jwt.sign(
			{ id: response.data.newUser.id, name: response.data.newUser.name },
			JWT_ACCESS,
			{ expiresIn: "15min" }
		);
...
export const login = async (
	...
	const response = await axios
		.post(
			`http://${process.env.USER_HOST}:${process.env.USER_PORT}/user/verify`,
			{
				...req.body,
			}
		)
