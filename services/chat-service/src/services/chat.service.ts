...
export class ChatService {
	...
	async getMessageBetweenUsers(user1Id: string, user2Id: string): Promise<Message[]> {
		return await this.messageRepository
			.createQueryBuilder('message')
			.where('(message.senderId = :user1Id AND message.receiverId = :user2Id)')
			.orWhere('(message.senderId = :user2Id AND message.receiverId = :user1Id)')
	...
	async markMessageAsRead(senderId: string, receiverId: string): Promise<void> {
		await this.messageRepository
			...
			.update(Message)
			.set({ isRead: true })
			.where('senderId = :senderId AND receiverId = :receiverId AND isRead = fasle')
