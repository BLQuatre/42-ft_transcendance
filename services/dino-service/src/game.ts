...
export class Game {
	...
	private handleObstacle() {
		const spawn =
			Math.random() <=
			0.01 + Math.log(1 + this.score / (1000 * CONST.DIFFICULTY_COEFF));
		...
		this.obstacles = this.obstacles.filter(
			(obstacle) => obstacle.getX().from >= 0
		); // Erase off-screen obstacles
	...
	addPlayer(player: Player) {
		this.dinos.push({ player: player, score: -1 });
	}
	...
	checkObstacles() {
		...
		this.obstacles.forEach((obstacle) => {
			...
			const first_dino_x = this.dinos.at(0)?.player.getX() ?? {
				from: 0,
				to: 0,
			}; // assuming every players share the same x (as it's not modifiable)
			// still using ?? logic to make following code lighter

			if (
				!(
					obstacle_x.to > first_dino_x.from && obstacle_x.from < first_dino_x.to
				)
			)
				// checking collision between obstacle and dino (x-axis wise)
				return;

			const obstacle_y = obstacle.getY();
			for (const dino of this.dinos) {
				if (dino.score > -1) continue;

				const dino_y = dino.player.getY();

				if (obstacle_y.to > dino_y.from && obstacle_y.from < dino_y.to) {
					// checking collision between obstacle and dino (y-axis wise)
	...
	private endGame() {
		...
		const highestScore = this.dinos.reduce(
