...
export class Obstacle {
	...
	constructor(type: 1 | 2 | 3 | 4, score: number) {
		...
		switch (type) {
			case CONST.TYPE_CACTUS:
				this.x = {
					from: CONST.SCREEN_WIDTH,
					to: CONST.SCREEN_WIDTH + CONST.CACTUS_WIDTH,
				};
			...
			case CONST.TYPE_GROUP:
				this.x = {
					from: CONST.SCREEN_WIDTH,
					to: CONST.SCREEN_WIDTH + CONST.GROUP_WIDTH,
				};
			...
			case CONST.TYPE_SMALL:
				this.x = {
					from: CONST.SCREEN_WIDTH,
					to: CONST.SCREEN_WIDTH + CONST.SMALL_WIDTH,
				};
			...
			default: // PTERO
				this.x = {
					from: CONST.SCREEN_WIDTH,
					to: CONST.SCREEN_WIDTH + CONST.PTERO_WIDTH,
				};
	...
	private autoUpdate() {
		...
		setInterval(() => {
			...
			const speed =
				CONST.OBSTCL_BASE_SPD +
				CONST.OBSTCL_BASE_SPD *
					Math.log(1.3 + this.score / CONST.SPD_SCALE_DIV);
			this.x.from -= speed;
			this.x.to -= speed;
		}, interval);
	}
