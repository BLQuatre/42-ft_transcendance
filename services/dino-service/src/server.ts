...
const start = async () => {
	...
	console.log(
		`Server running on http://${process.env.DINO_HOST}:${process.env.DINO_PORT}`
	);
	...
	wss.on("connection", (ws) => {
		...
		ws.on("message", (message) => {
			...
			if (data.type === "assign") {
				...
				(data.type === "jump" || data.type === "up" || data.type === "down") &&
				data.playerId
			) {
				if (assignedPlayer?.getId() !== data.playerId) return;

				if (data.type === "jump") assignedPlayer?.jump();
				else if (data.type === "up") assignedPlayer?.stand();
