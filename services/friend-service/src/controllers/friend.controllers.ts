...
export const createFriend = async (
	...
	const findSender = await axios.get(
		`http://${process.env.USER_HOST}:${process.env.USER_PORT}/user/${senderId}`
	);
	const findReceiver = await axios.get(
		`http://${process.env.USER_HOST}:${process.env.USER_PORT}/user/${id}`
	);
	if (
		findSender.data.statusCode !== 200 &&
		findReceiver.data.statusCode !== 200
	) {
		return reply.code(404).send({
...
export const getSendingPendingRequest = async (
	req: FastifyRequest,
	reply: FastifyReply
) => {
	const id = req.headers["x-user-id"] as string;
...
export const getReceivingPendingRequest = async (
	req: FastifyRequest,
	reply: FastifyReply
) => {
	const id = req.headers["x-user-id"] as string;
...
export const responseFriend = async (
	req: FastifyRequest<{ Body: { status: FriendRequestStatus } }>,
	reply: FastifyReply
) => {
	...
	const user = await axios.get(
		`http://${process.env.USER_HOST}:${process.env.USER_PORT}/user/${userId}`
	);
...
export const blockFriend = async (req: FastifyRequest, reply: FastifyReply) => {
	...
	const senderOnreceiver = await Friend.findOneBy({
		sender_id: id,
...
export const getBlockedList = async (
	...
	const friends = await Friend.find({
		where: [{ sender_id: id, status: FriendRequestStatus.BLOCKED }],
	});
	return reply.code(200).send({
