...
export const createGameSession = async (
	...
	return reply
		.code(201)
		.send({ message: "Game session created", statusCode: 201 });
};

export const getAllMyGamehistory = async (
	req: FastifyRequest,
	reply: FastifyReply
) => {
	const { user_id } = req.params as { user_id: string };

	const history = await ResultGame.createQueryBuilder("result")
	...
	return reply
		.code(200)
		.send({ message: "All our match histroy", statuCode: 200, history });
};
...
export const getStats = async (req: FastifyRequest, reply: FastifyReply) => {
	...
	const totalGames = await ResultGame.createQueryBuilder("result")
		.leftJoin("result.player", "player")
		.where("player.user_id = :user_id", { user_id })
		.getCount();

	// 2. Taux de victoire sur PONG
	const pongVictories = await ResultGame.createQueryBuilder("result")
		.leftJoin("result.player", "player")
	...
	const pongTotal = await ResultGame.createQueryBuilder("result")
		.leftJoin("result.player", "player")
		.leftJoin("result.gameSession", "session")
		.where("player.user_id = :user_id", { user_id })
		.andWhere("session.game_type = :type", { type: "PONG" })
	...
	const bestDinoScoreRow = await ResultGame.createQueryBuilder("result")
		.leftJoin("result.player", "player")
		.leftJoin("result.gameSession", "session")
		.where("player.user_id = :user_id", { user_id })
